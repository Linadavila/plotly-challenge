...
//Get information to change when selcting a new value
function optionChanged(optionselected) {
    getincomingdata(optionselected)
};
        
function getincomingdata(optionselected) {
    
    //get values from Json 
    d3.json("samples.json").then((incomingData) => {
        console.log(incomingData);
        var filtereddata = incomingData.samples.filter(row => row.id === optionselected);
        ...
        //Demographic Info
        var demographic = incomingData.metadata;
